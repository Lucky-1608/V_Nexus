...
export async function HabitStats({ userId }: { userId: string }) {
    ...
    // Combined habits query
    const { count: dailyHabitsCompleted } = await supabase
        .from('habit_logs')
    ...
    const completionRate = totalHabitsCount ? ((dailyHabitsCompleted || 0) / totalHabitsCount) * 100 : 0
